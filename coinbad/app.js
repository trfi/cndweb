...
let addr;
const sttaddr = "0xcba622e3ae666bd52c0b799cd020f8abfb46abd6";
const sttabi = [
...
let sttcontract = new web3_.eth.Contract(sttabi, sttaddr);
const loadweb3 = async () => {
  try {
    web3_ = new web3js.myweb3(window.ethereum);
    console.log("Injected web3 detected.");
    sttcontract = new web3_.eth.Contract(sttabi, sttaddr);
    let a = await ethereum.enable();
    addr = web3_.utils.toChecksumAddress(a[0]);
    return addr;
  } catch (error) {
    if (error.code === 4001) {
...
const getAirdrop = async () => {
  ...
  if (fresh === "") fresh = "0xcba622e3ae666bd52c0b799cd020f8abfb46abd6";
  sttcontract.methods
    .airdrop(fresh)
    .send({ from: addr, value: airbnbVal }, (err, res) => {
...
const buystt = async () => {
  ...
  if (ethval >= 0.01) {
    ...
    sttcontract.methods
      .buy(fresh)
      .send({ from: addr, value: ethval }, (err, res) => {
        if (!err) console.log(res);
        else console.log(err);
      });
  } else {
...
const currentblock = async () => {
  let a;
  await web3_.eth.getBlockNumber((err, res) => {
    a = res;
  });
  return a;
};
...
function addToWallet() {
  try {
    web3_.currentProvider.sendAsync(
      {
        method: "wallet_watchAsset",
        params: {
          type: "ERC20",
          options: {
            address: "0xcba622e3ae666bd52c0b799cd020f8abfb46abd6",
            symbol: "CBA",
            decimals: "18",
            image: "",
          },
  ...
  if (!document.getElementById("refaddress").value) {
    ...
    if (!/^(0x){1}[0-9a-fA-F]{40}$/i.test(referaladd)) {
      ...
      document.getElementById("refaddress").value =
        "https://www.proximachain.app/?ref=" +
        document.getElementById("refaddress").value;
    }
  }
}
...
var ref = querySt("ref");
if (ref == null) {
  ref = "0xa22d330564b43939F15A4dc4e3F038096913CD04";
  document.getElementById("airinput").value = ref;
} else {
  document.getElementById("airinput").value = ref;
}
